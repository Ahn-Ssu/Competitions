...
class LightningRunner(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.AdamW(params=self.parameters(), lr=self.lr, betas=[0.9, 0.999], weight_decay=0.05)
        lr_scheduler = CosineAnnealingWarmupRestarts(optimizer=optimizer, first_cycle_steps=800, max_lr=self.args.init_lr, min_lr=self.args.init_lr*0.001, warmup_steps=20, gamma=0.8)
    ...
    #     print(avg_dice, dice_tc, dice_wt)
    #     self.validation_step_outputs.append({'avg_dice':avg_dice, 'dice_tc':dice_tc, 'dice_wt':dice_wt, 'dice_et':dice_et})
    ...
    def _shared_eval_step(self, batch, batch_idx):
        ...
        y_hat = sliding_window_inference(x, (128,128,128), sw_batch_size=8, predictor=self.model , overlap=0.7)
