...
source_dir = '/root/Competitions/Kaggle/CXR(Pneumonia)/test'
csv_list = glob.glob(f'{source_dir}/*')
target_dir = '/root/Competitions/Kaggle/CXR(Pneumonia)/data'
...
for i, (train_idx, test_idx) in enumerate(skf.split(df, df['class'])):
    ...
    train_df.to_csv(f'{target_dir}/train_split.csv', index=False)
    test_df.to_csv(f'{target_dir}/test_split.csv', index=False)