...
rdkit_df = pd.read_csv('/root/Competitions/DACON/4. JUMP AI 2023/data/rdkit_test.csv').iloc[:, 1:].to_numpy()
features = np.concatenate([features, rdkit_df], axis=1)
...
hparams_path = '/root/Competitions/DACON/4. JUMP AI 2023/lightning_logs/2.ChemBERT/2023-09-07/ChemBERT(arch1)/4/hparams.yaml'
...
ckpt_root = '/root/Competitions/DACON/4. JUMP AI 2023/lightning_logs/2.ChemBERT/2023-09-07/ChemBERT(arch1)'
ckpts = glob.glob(f'{ckpt_root}/*/checkpoints/ChemBERT*')
...
submit.to_csv(f'{output_path}/ChemBERT(1)x5.csv', index=False)